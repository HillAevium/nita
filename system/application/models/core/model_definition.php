...
abstract class Model_Definition implements Has_Fields {
    ...
    public function isRequired(Field $field) {
        ...
        log_message('error', print_r($this->dependantFields, true));
        log_message('error', print_r($this->requiredFields, true));
        log_message('error', print_r($this->optionalFields, true));
        log_message('error', print_r($field, true));
    ...
    public function processPost($returnType = 'object') {
        ...
        log_message('error', 'processing ' . count($this->fields()) . ' fields');
        foreach($this->fields() as $field) {
            log_message('error', $field->name());
            if($field->isEmpty()) {
                ...
                log_message('error', ' - store');
                $returnData[$field->name()] = $field->process();
            }
        }
        log_message('error', 'endforeach');
